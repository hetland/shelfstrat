...
parser.add_argument('--tidx', type=int, default=0, help='time index to plot')
...
sss_ticks = np.linspace(sss.min(), sss.max(), 20)
cf_salt = ax.contourf(xx, yy, sss, sss_ticks, cmap=salt_cmap)
ax.contour(xx, yy, sss, sss_ticks, linewidths=0.2)
...
plt.savefig('sss_%s_%d.pdf' % (casedir, tidx))
